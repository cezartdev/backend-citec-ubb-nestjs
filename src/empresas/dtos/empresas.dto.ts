import { IsString, IsNotEmpty, IsEmail, Length, IsIn, Matches, IsNumber, IsOptional, ValidateNested, IsArray } from 'class-validator';
...
export class ActualizarEmpresasDto {
    ...
    @ApiProperty({ description: 'Este es el rut de la empresa' })
    readonly rut: string;
    ...
    @Length(1, 100, {
        message: 'La longitud de la razon social debe ser entre 1 y 80 caracteres',
    })
    ...
    @Length(1, 100, {
        message: 'La longitud de la nombre de fantasia debe ser entre 1 y 100 caracteres',
    })
    ...
    @ApiProperty({ description: 'Este es el email de la empresa', default: 'juan@gmail.com' })
    ...
    @ApiProperty({ description: 'Este es la direccion de la empresa', default: 'Calle Bulnes 1080 N°10' })
    ...
    @Length(1, 30, {
        message:
            'La longitud del estado debe ser entre 1 y 30 caracteres',
    })
    ...
    @ApiProperty({ description: 'Este es el estado de la empresa', default: 'ACTIVO' })
    ...
    @Length(1, 20, {
        message:
            'La longitud del telefono debe ser entre 1 y 20 caracteres',
    })
    @Matches(telefonoRegex, { message: 'El telefono debe ser un telefono valido' })
    @IsString({ message: 'El telefono debe ser texto' })
    @IsOptional()
    @ApiProperty({ description: 'Este es el telefono de la empresa', default: '+56912345678' })
    ...
    @ApiProperty({ description: 'Estos son los giros de la empresa', default: [11101, 11102, 11103] })
    readonly giros: number[];
}

export class CrearEmpresasDto extends OmitType(ActualizarEmpresasDto, ['nuevo_rut']) {}

export class ObtenerPorIdEmpresasDto extends PickType(ActualizarEmpresasDto, ['rut']) {}

export class EliminarEmpresasDto extends PickType(ActualizarEmpresasDto, ['rut']) {}

export class RetornoEmpresasDto extends OmitType(ActualizarEmpresasDto, ['nuevo_rut', 'id_comunas', 'giros', 'contactos']) {

    @ApiProperty({ description: 'Esta es la comuna de la empresa' })
    readonly id_comunas: Comunas;

    @ApiProperty({ description: 'Estos son los giros de la empresa' })
    readonly giros: Giros[];

    @ApiProperty({ description: 'Estos son los contactos de la empresa' })
